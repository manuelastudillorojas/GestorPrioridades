...
const addSeveridad = async (req, res) => {
  try {
    if (validaVacio(req.body.nombre) && validaVacio(req.body.puntos) && validaVacio(req.body.id_criticidad)) {

      const severidad = await severidadModel.addSeveridad(req.body)
      sendJsonStatus(res, 200, {
        msg: {
...
const updateSeveridad = async (req, res) => {
  try {
    if (validaVacio(req.params.id) && validaVacio(req.body.nombre) && validaVacio(req.body.puntos) && validaVacio(req.body.id_criticidad)) {
      const severidad = await severidadModel.updateSeveridad(req.params.id, req.body)
      sendJsonStatus(res, 200, {
