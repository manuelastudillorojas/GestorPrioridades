...
const getCriticidad = async (req, res) => {
  try {
    ...
    if (criticidad.rows.length == 0) {
      ...
      sendJsonStatus(res, 200, { datos: criticidad.rows });
    }
  } catch (error) {
...
const createCriticidad = async (req, res) => {
  try {
    if (
      ...
      const criticidad = await criticidadModel.createCriticidad(req.body);
      sendJsonStatus(res, 200, {
        msg: { info: `Se han ingresado correctamente`, datos: criticidad.rows },
        tipo: `ok`,
      });
    } else {
      sendJsonStatus(res, 200, {
...
const updateCriticidad = async (req, res) => {
  try {
    if (
      ...
      const criticidad = await criticidadModel.updateCriticidad(
        req.params.id,
        req.body
      );
      sendJsonStatus(res, 200, {
