...
const getDeadline = async (req, res) => {
  try {
    ...
    if (deadline.rows.length == 0) {
      ...
      sendJsonStatus(res, 200, { datos: deadline.rows });
    }
  } catch (error) {
...
const createDeadline = async (req, res) => {
  try {
    if (
      validaVacio(req.body.id_severidad) &&
      validaVacio(req.body.max_min) &&
...
const updateDeadline = async (req, res) => {
  try {
    if (
      ...
      const deadline = await deadlineModel.updateDeadline(
        req.params.id,
        req.body
      );
      sendJsonStatus(res, 200, {
