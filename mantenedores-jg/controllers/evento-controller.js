...
const getEvents = async (req, res) => {
    ...
    try{
        ...
        if (eventos.rows.length == 0) {
            sendJsonStatus(res, 200, {msg : `No hay eventos registrados`, tipo: `error`})
        } else {
            sendJsonStatus(res, 200, {datos: eventos.rows}) 
        }
    } catch (error) {
    ...
    try {
        if(validaVacio(req.params.id)){
            ...
            if(evento.rows.length == 0){
                sendJsonStatus(res, 200, {msg : `No hay eventos registrados con el id ${req.params.id}`, tipo: `error`})
            } else {
                sendJsonStatus(res, 200, {datos: evento.rows})
            }
          } else {
    ...
    try {
        ...
        if(validaVacio(req.body.descripcion) && validaVacio(req.body.hora_registro) && validaVacio(req.body.estado) && validaVacio(req.body.puntaje_prio) && validaVacio(req.body.id_cliente)
            && validaVacio(req.body.id_componente) && validaVacio(req.body.id_criticidad) && validaVacio(req.body.id_severidad)) {
                ...
                sendJsonStatus(res, 200, {msg: {info: `Se han ingresado correctamente`, datos: evento.rows}, tipo: `ok`})
    ...
    try {
        if(validaVacio(req.params.id) && validaVacio(req.body.descripcion) && validaVacio(req.body.hora_registro) && validaVacio(req.body.estado) && validaVacio(req.body.puntaje_prio) && validaVacio(req.body.id_cliente)
            && validaVacio(req.body.id_componente) && validaVacio(req.body.id_criticidad) && validaVacio(req.body.id_severidad)){
                ...
                sendJsonStatus(res, 200, {msg: `Datos actualizados en ${evento.rowCount} filas`, tipo: `ok`})
            } else {
                sendJsonStatus(res, 200, {msg: `Todos los datos deben contener informacion`, tipo: `error`})
            }
    } catch (error) {
 ...
 const deleteEvent = async (req, res) => {
    try {
        if(validaVacio(req.params.id)) {
            ...
            sendJsonStatus(res, 200, {msg: `Cantidad de filas eliminadas: ${evento.rowCount}`, tipo: `ok`})
        } else {
            sendJsonStatus(res, 200, {msg: `Id vacío`, tipo: `error`})
        }
    } catch (error) {
