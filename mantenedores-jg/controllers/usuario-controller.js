...
const getUsuarios = async (req, res) => {
    ...
    try{
        ...
        if (usuarios.rows.length == 0) {
            sendJsonStatus(res, 200, {msg : `No hay usuarios registrados`, tipo: `error`})
        } else {
            sendJsonStatus(res, 200, {datos: usuarios.rows}) 
        }
    } catch (error) {
    ...
    try {
        if(validaVacio(req.params.mail)){
            ...
            if(usuario.rows.length == 0){
                sendJsonStatus(res, 200, {msg : `No hay usuarios registrados con el correo ${req.params.mail}`, tipo: `error`})
            } else {
                sendJsonStatus(res, 200, {datos: usuario.rows})
            }
          } else {
            sendJsonStatus(res, 200, {msg: `Correo vacío`, tipo: `error`})
          }
    ...
    try {
        ...
        if(validaVacio(req.body.correo) && validaVacio(req.body.contrasena) && validaVacio(req.body.rol)) {
                ...
                if(validaCorreoDuplicado.rowCount == 0) {
                    ...
                    sendJsonStatus(res, 200, {msg: {info: `Se han ingresado correctamente`, datos: usuario.rows}, tipo: `ok`})
                } else {
                    sendJsonStatus(res, 300, {msg: `Correo ya está registrado`, tipo: `error`})
                }
            } else {
                sendJsonStatus(res, 300, {msg: `Todos los datos deben contener informacion`, tipo: `error`})
            }
 ...
 const updateUsuario = async (req, res) => {
    try {
        if(validaVacio(req.params.mail) && validaVacio(req.body.contrasena) && validaVacio(req.body.rol)){
                ...
                sendJsonStatus(res, 200, {msg: `Datos actualizados en ${usuario.rowCount} filas`, tipo: `ok`})
            } else {
                sendJsonStatus(res, 200, {msg: `Todos los datos deben contener informacion`, tipo: `error`})
            }
    } catch (error) {
        sendJsonStatus(res, 500, error.message)
    }
 ...
 const deleteUsuario = async (req, res) => {
    try {
        if(validaVacio(req.params.mail)) {
            ...
            sendJsonStatus(res, 200, {msg: `Cantidad de filas eliminadas: ${usuario.rowCount}`, tipo: `ok`})
        } else {
            sendJsonStatus(res, 200, {msg: `Correo vacío`, tipo: `error`})
        }
    } catch (error) {
